...
###########################
#对ctDNA样本的fastq或者fq进行处理，然后采用impact修正后的方法完成这个任务，得到想要的ctDNA突变。之前samtools call变异结果对indel不是太好，现在改用varscan2来call。
...
close INFILE2;
#####记录程序执行信心
my $kaishi=system("script $Tumor_R1_short.log.txt");
...
###改用VarScan进行变异call取。
my $hv = system("samtools mpileup -C 100 -AB -d 10000 -q 60 -Q 30 -f $path_to_ref_exome/hg19_exome.fa $Tumor_pic_rd2_output | java -jar $path_to_VarScan/VarScan.v2.3.9.jar mpileup2cns --min-var-freq 0 --p-value 0.99 --min-avg-qual 15  --min-coverage 8 --min-reads2 1  --output-vcf 1 > $Tumor_VCF ");

my $i = system(" ./ct-DNA-varscan2-filter.pl $Tumor_VCF  > $Tumor_cleaned ");
my $j = system(" convert2annovar.pl -format vcf4 --includeinfo $Tumor_cleaned > $Tumor_avi ");
my $k = system(" table_annovar.pl $Tumor_avi $path_to_annovar_humandb -buildver hg19 -out $Tumor_annovar -remove -protocol knowngene,refGene,cosmic70,dbnsfp30a,esp6500si_all,mcap,1000g2014oct_all,clinvar_20150629,EAS.sites.2015_08,snp138,exac03 -operation g,g,f,f,f,f,f,f,f,f,f ");
