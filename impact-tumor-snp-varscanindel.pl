...
###########################
#对肿瘤样本的fastq或者fq进行处理，然后采用impact修正后的方法完成这个任务，得到想要的体细胞突变。之前samtools call snp，现在改用varscan2来call indel。
...
my $Tumor_VCF1 = "$Tumor_R1_short".".impact.vcf";
my $Tumor_VCF2 = "$Tumor_R1_short".".varscan.vcf";
...
my $hg = system("samtools mpileup -B -d 10000 -v -u -f $path_to_ref_exome/hg19_exome.fa $Tumor_pic_rd2_output | $path_to_bcftools/bcftools call -O v -v -c -n 0.05 -p 1 -A -o $Tumor_VCF1 ");
my $hv = system("samtools mpileup -B -d 10000  -f $path_to_ref_exome/hg19_exome.fa $Tumor_pic_rd2_output | java -jar $path_to_VarScan/VarScan.v2.3.9.jar mpileup2indel --output-vcf 1 > $Tumor_VCF2 ");
my $ig = system(" ./change_gt_values.pl $Tumor_VCF1  > $Tumor_cleaned ");
my $iv = system(" ./tumor-DNA-varscan2-filter.pl $Tumor_VCF2  >> $Tumor_cleaned ");
my $j = system(" convert2annovar.pl -format vcf4 --includeinfo $Tumor_cleaned | uniq > $Tumor_avi ");
